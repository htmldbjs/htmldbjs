<?php
/**
 * generateDateSQLCriteria - generates date property sql criteria
 *
...
function generateDateSQLCriteria($property, $filters, $connection, $criteriaSQL) {
    ...
    if (isset($filters[$property . 'InValues'])) {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $criteriaSQL .= '\''
                    . date('Y-m-d', intval($filters[$property . 'InValues'][$i]))
                    . '\'';
        } // for ($i = 0; $i < $count; $i++) {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $criteriaSQL .= '\''
                    . date('Y-m-d', intval($filters[$property . 'NotInValues'][$i]))
                    . '\'';
        } // for ($i = 0; $i < $count; $i++) {
        ...
        if (isset($filters[$property . 'MinExclusive'])) {
            ...
            $criteriaSQL .= '(`' . $tableField . '` > \''
                    . date('Y-m-d', $filters[$property . 'MinExclusive'])
                    . '\')';
            ...
            $criteriaSQL .= '(`' . $tableField . '` >= \''
                    . date('Y-m-d', $filters[$property . 'MinInclusive'])
                    . '\')';
        ...
        if (isset($filters[$property . 'MaxExclusive'])) {
            ...
            $criteriaSQL .= '(`' . $tableField . '` < \''
                            . date('Y-m-d', $filters[$property . 'MaxExclusive'])
                            . '\')';
            ...
            $criteriaSQL .= '(`' . $tableField . '` <= \''
                            . date('Y-m-d', $filters[$property . 'MaxInclusive'])
                            . '\')';
