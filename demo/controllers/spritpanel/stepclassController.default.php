...
class stepclassController {

	public $controller = 'stepclass';
	...
	public function index($parameters = NULL, $strMethod = '') {
        ...
        if (!includeView($this, 'spritpanel/stepclass')) {
			includeLibrary('spritpanel/redirectToPage');
	...
	public function read($parameters = NULL) {
		...
		for ($i = 0; $i < $objectCount; $i++) {
			...
			$this->list[$index]['checklist_id'] = $object->checklist_id;
			$this->list[$index]['checklist_idDisplayText']
					= $object->getForeignDisplayText('checklist_id');
			$this->list[$index]['steptype_id'] = $object->steptype_id;
			$this->list[$index]['steptype_idDisplayText']
					= $object->getForeignDisplayText('steptype_id');
			$this->list[$index]['action'] = $object->action;
			$this->list[$index]['detection'] = $object->detection;
			...
			$this->list[$index]['stepstate_id'] = $object->stepstate_id;
			$this->list[$index]['stepstate_idDisplayText']
					= $object->getForeignDisplayText('stepstate_id');
			$index++;
		...
		$this->columns = array();
		$this->columns[] = 'id';
		$this->columns[] = 'checklist_id';
		$this->columns[] = 'checklist_idDisplayText';
		$this->columns[] = 'steptype_id';
		$this->columns[] = 'steptype_idDisplayText';
		$this->columns[] = 'action';
		$this->columns[] = 'detection';
		$this->columns[] = 'photo';
		...
		$this->columns[] = 'stepstate_id';
		$this->columns[] = 'stepstate_idDisplayText';
	...
	public function readsession($parameters = NULL) {
		...
		$this->columns[] = 'stepstate_idSearchText';
	...
	public function writesession($parameters = NULL) {
		...
		if (isset($_REQUEST['inputaction0'])
			...
			$stepstate_idSearchText = isset($_REQUEST['inputfield0stepstate_idSearchText'])
					? htmlspecialchars($_REQUEST['inputfield0stepstate_idSearchText'])
					: $sessionParameters['stepstate_idSearchText'];
			$_SESSION[sha1(__FILE__) . 'stepstate_idSearchText'] = $stepstate_idSearchText;
	...
	private function getSessionParameters() {
		...
		$sessionParameters['pageCount']
				...
				: 0);
		$sessionParameters['checklist_idSearchText']
				= (isset($_SESSION[sha1(__FILE__) . 'checklist_idSearchText'])
				? $_SESSION[sha1(__FILE__) . 'checklist_idSearchText']
				: '');
		$sessionParameters['steptype_idSearchText']
				= (isset($_SESSION[sha1(__FILE__) . 'steptype_idSearchText'])
				? $_SESSION[sha1(__FILE__) . 'steptype_idSearchText']
				: '');
		$sessionParameters['stepstate_idSearchText']
				= (isset($_SESSION[sha1(__FILE__) . 'stepstate_idSearchText'])
				? $_SESSION[sha1(__FILE__) . 'stepstate_idSearchText']
				: '');
		$sessionParameters['currentDirectory']
