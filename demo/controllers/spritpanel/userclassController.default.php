...
class userclassController {
	...
	public function index($parameters = NULL, $strMethod = '') {
        ...
        if (!includeView($this, 'spritpanel/userclass')) {
			includeLibrary('spritpanel/redirectToPage');
	...
	public function read($parameters = NULL) {
		...
		for ($i = 0; $i < $objectCount; $i++) {
			...
			$this->list[$index]['user_type'] = $object->user_type;
			$this->list[$index]['company'] = $object->company;
			$this->list[$index]['companyDisplayText']
					= $object->getForeignDisplayText('company');
		...
		$this->columns[] = 'user_type';
		$this->columns[] = 'company';
		$this->columns[] = 'companyDisplayText';
	...
	public function readsession($parameters = NULL) {
		...
		$this->columns[] = 'companySearchText';
	...
	public function writesession($parameters = NULL) {
		...
		if (isset($_REQUEST['inputaction0'])
			...
			$companySearchText = isset($_REQUEST['inputfield0companySearchText'])
					? htmlspecialchars($_REQUEST['inputfield0companySearchText'])
					: $sessionParameters['companySearchText'];
			$_SESSION[sha1(__FILE__) . 'companySearchText'] = $companySearchText;
	...
	private function getSessionParameters() {
		...
		$sessionParameters['pageCount']
				...
				: 0);
		$sessionParameters['companySearchText']
				= (isset($_SESSION[sha1(__FILE__) . 'companySearchText'])
				? $_SESSION[sha1(__FILE__) . 'companySearchText']
				: '');
