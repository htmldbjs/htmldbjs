...
class coordinatorclassController {

	public $controller = 'coordinatorclass';
	...
	public function index($parameters = NULL, $strMethod = '') {
        ...
        if (!includeView($this, 'spritpanel/coordinatorclass')) {
			includeLibrary('spritpanel/redirectToPage');
	...
	public function read($parameters = NULL) {
		...
		for ($i = 0; $i < $objectCount; $i++) {
			...
			$this->list[$index]['firm_id'] = $object->firm_id;
			$this->list[$index]['firm_idDisplayText']
					= $object->getForeignDisplayText('firm_id');
		...
		$this->columns[] = 'firm_id';
		$this->columns[] = 'firm_idDisplayText';
	...
	public function readsession($parameters = NULL) {
		...
		$this->columns[] = 'firm_idSearchText';
	...
	public function writesession($parameters = NULL) {
		...
		if (isset($_REQUEST['inputaction0'])
			...
			$firm_idSearchText = isset($_REQUEST['inputfield0firm_idSearchText'])
					? htmlspecialchars($_REQUEST['inputfield0firm_idSearchText'])
					: $sessionParameters['firm_idSearchText'];
			$_SESSION[sha1(__FILE__) . 'firm_idSearchText'] = $firm_idSearchText;
	...
	private function getSessionParameters() {
		...
		$sessionParameters['pageCount']
				...
				: 0);
		$sessionParameters['firm_idSearchText']
				= (isset($_SESSION[sha1(__FILE__) . 'firm_idSearchText'])
				? $_SESSION[sha1(__FILE__) . 'firm_idSearchText']
				: '');
