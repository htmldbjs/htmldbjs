...
class crewmemberclassController {

	public $controller = 'crewmemberclass';
	...
	public function index($parameters = NULL, $strMethod = '') {
        ...
        if (!includeView($this, 'spritpanel/crewmemberclass')) {
			includeLibrary('spritpanel/redirectToPage');
	...
	public function read($parameters = NULL) {
		...
		for ($i = 0; $i < $objectCount; $i++) {
			...
			$this->list[$index]['crew_id'] = $object->crew_id;
			$this->list[$index]['crew_idDisplayText']
					= $object->getForeignDisplayText('crew_id');
			$this->list[$index]['leader'] = $object->leader;
			$this->list[$index]['firstname'] = $object->firstname;
			$this->list[$index]['lastname'] = $object->lastname;
			$this->list[$index]['email'] = $object->email;
			$this->list[$index]['phone'] = $object->phone;
			$this->list[$index]['password'] = $object->password;
			$index++;
		...
		$this->columns[] = 'crew_id';
		$this->columns[] = 'crew_idDisplayText';
		$this->columns[] = 'leader';
	...
	public function readsession($parameters = NULL) {
		...
		$this->columns[] = 'crew_idSearchText';
	...
	public function writesession($parameters = NULL) {
		...
		if (isset($_REQUEST['inputaction0'])
			...
			$crew_idSearchText = isset($_REQUEST['inputfield0crew_idSearchText'])
					? htmlspecialchars($_REQUEST['inputfield0crew_idSearchText'])
					: $sessionParameters['crew_idSearchText'];
			$_SESSION[sha1(__FILE__) . 'crew_idSearchText'] = $crew_idSearchText;
	...
	private function getSessionParameters() {
		...
		$sessionParameters['pageCount']
				...
				: 0);
		$sessionParameters['crew_idSearchText']
				= (isset($_SESSION[sha1(__FILE__) . 'crew_idSearchText'])
				? $_SESSION[sha1(__FILE__) . 'crew_idSearchText']
				: '');
