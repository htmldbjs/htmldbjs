...
/**
 * CLASS AUDITSTEPTYPE
 * Implements AuditStepType Class properties and methods and
 * handles AuditStepType Class database transactions.	
 */
...
class AuditStepType {
    ...
    public function install() {
        ...
        // Update MySQL Table
        $SQLText = 'SHOW TABLES LIKE "auditsteptypetable"';
        ...
        if ($result->num_rows > 0) {
            ...
            $backupTableName = ('bck_auditsteptypetable' . date('YmdHis'));
            $SQLText = 'CREATE TABLE `'
                    . $backupTableName
                    . '` LIKE `auditsteptypetable`;';
            $this->__mySQLConnection->query($SQLText);
            $SQLText = 'INSERT `'
                    . $backupTableName
                    . '` SELECT * FROM `auditsteptypetable`;';
            $this->__mySQLConnection->query($SQLText);

        } else {

            // Create Table If Not Exists
            $SQLText = 'CREATE TABLE `auditsteptypetable` (
		...
		// index
		$strSQL = 'SHOW COLUMNS FROM `auditsteptypetable`'
				. ' LIKE "index";';
		$result = $this->__mySQLConnection->query($strSQL);
		$strSQL = 'ALTER TABLE `auditsteptypetable`'
				. (($result) ? ' ADD ' : ' MODIFY ')
	...
	/**
     * assign - Copies a AuditStepType instance to this instance.
     *
     * @param objAuditStepType [AuditStepType][in]: AuditStepType instance to be copied
     */
     ...
     *
     * @return Returns newly created AuditStepType id on success, false on failure.
     */
	public function insert() {
		...
		$SQLText = 'INSERT INTO `auditsteptypetable` '
				. '(`deleted`,'
				. '`creationdate`,'
				. '`lastupdate`'
				. ', `index`'
		...
		$SQLText = str_replace('{{parameter0}}', intval($this->index), $SQLText);
		$SQLText = str_replace('{{parameter1}}', intval($this->enabled), $SQLText);
	...
	public function update() {
		...
		$SQLText = 'UPDATE `auditsteptypetable` SET '
				. '`deleted`={{deleted}},'
				. '`lastupdate`=NOW() '
				. ', `index`=\'{{parameter0}}\' '
				. ', `enabled`=\'{{parameter1}}\' '
	...
	public function revert($fromCache = false) {
    	...
    	if ($fromCache && ($this->id > 0)) {
			...
			$SQLText = 'SELECT * FROM `auditsteptypetable` WHERE `id`={{id}};';
			$SQLText = str_replace('{{id}}', intval($this->id), $SQLText);
	...
	public function delete() {
		...
		if ($this->deleted) {
			$SQLText = 'DELETE FROM `auditsteptypetable` '
					. ' WHERE `id`={{id}};';
		} else {
			$SQLText = 'UPDATE `auditsteptypetable` SET '
					. '`deleted`=1'
	...
	public function read() {
		...
		$cacheFile = (DBDIR . '/AuditStepType/' . intval($this->id) . '.php');
	...
	public function write() {
	    ...
	    if (0 == $this->id) {
	        ...
	        if (file_exists(DBDIR . '/AuditStepType/__id')) {
	            $this->id = file_get_contents(DBDIR . '/AuditStepType/__id');
	        } // if (file_exists(DBDIR . '/AuditStepType/__id')) {
	        ...
	        while(file_exists(DBDIR . '/AuditStepType/' . $this->id . '.php')
	                || file_exists(DBDIR . '/AuditStepType/--' . $this->id . '.php')) {
	            $this->id++;
	        } // while(file_exists(DBDIR . '/AuditStepType/' . $this->id . '.php')
	     
	        includeLibrary('writeStringToFileViaFTP');
	        writeStringToFileViaFTP('Database/AuditStepType/__id', $this->id);	     
	    } // if (0 == $this->id) {
        ...
        $cacheFile = ('Database/AuditStepType/' . $this->id . '.php');
	...
	public function remove() {
		...
		$cacheFile = (FTP_PRIMARY_HOME . '/Database/AuditStepType/' . $this->id . '.php');
		$newCacheFile = (FTP_PRIMARY_HOME . '/Database/AuditStepType/--' . $this->id . '.php');
	...
	public function cache() {
		...
		if (!$this->deleted) {
			...
			if (file_exists(DIR . '/events/onAuditStepTypeCache.php')) {
				require_once(DIR . '/events/onAuditStepTypeCache.php');
				onAuditStepTypeCache($this, $this->id, $propertyValues, $bulk);
			} // if (file_exists(DIR . '/events/onAuditStepTypeCache.php')) {
			...
			if (file_exists(DIR . '/events/onAuditStepTypeUncache.php')) {
				require_once(DIR . '/events/onAuditStepTypeUncache.php');
				onAuditStepTypeUncache($this, $this->id, $bulk);
			} // if (file_exists(DIR . '/events/onAuditStepTypeUncache.php')) {
    ...
    /**
     * find - Finds AuditStepType instances specified with the listing
     * criteria
     *
     * @return Returns true on success, false on failure.
     */
	...
	public function doBeforeInsert() {
		...
		if (file_exists(DIR . '/events/onBeforeAuditStepTypeInsert.php')) {

			require_once(DIR . '/events/onBeforeAuditStepTypeInsert.php');
			onBeforeAuditStepTypeInsert($this);
	...
	public function doAfterInsert() {
		...
		if (file_exists(DIR . '/events/onAfterAuditStepTypeInsert.php')) {

			require_once(DIR . '/events/onAfterAuditStepTypeInsert.php');
			onAfterAuditStepTypeInsert($this);
	...
	public function doBeforeUpdate() {
		...
		if (file_exists(DIR . '/events/onBeforeAuditStepTypeUpdate.php')) {

			require_once(DIR . '/events/onBeforeAuditStepTypeUpdate.php');
			onBeforeAuditStepTypeUpdate($this);
	...
	public function doAfterUpdate() {
		...
		if (file_exists(DIR . '/events/onAfterAuditStepTypeUpdate.php')) {

			require_once(DIR . '/events/onAfterAuditStepTypeUpdate.php');
			onAfterAuditStepTypeUpdate($this);
	...
	public function doBeforeDelete() {
		...
		if (file_exists(DIR . '/events/onBeforeAuditStepTypeDelete.php')) {

			require_once(DIR . '/events/onBeforeAuditStepTypeDelete.php');
			onBeforeAuditStepTypeDelete($this);
	...
	public function doAfterDelete() {
		...
		if (file_exists(DIR . '/events/onAfterAuditStepTypeDelete.php')) {

			require_once(DIR . '/events/onAfterAuditStepTypeDelete.php');
			onAfterAuditStepTypeDelete($this);
	...
	private function writeFilterCache() {
		...
		$success = writeIntegerFilterCache('AuditStepType',
				$this->id,
				'deleted',
				$this->deleted);
	    if (!$success) {
		...
		$success = writeStringFilterCache('AuditStepType',
				$this->id,
				'name',
				$this->name);
	    if (!$success) {
	...
	private function removeFilterCache($removeOriginalFilters = true) {
        ...
        if ($removeOriginalFilters) {
		    $cacheFile = (DBDIR . '/AuditStepType/' . $this->id . '.php');
	    ...
	    $success = removeIntegerFilterCache('AuditStepType',
				$current->id,
				'deleted',
				$current->deleted);
	    if (!$success) {
		...
		$success = removeStringFilterCache('AuditStepType',
				$current->id,
				'name',
				$current->name);	
	    if (!$success) {
    ...
    /**
     * generateListFromFile - Generates list of AuditStepType instances from file
    ...
    private function generateListFromFile() {
        ...
        if (($this->__filters['lastUpdateMinExclusive'] > 0)
            ...
            includeLibrary('extractIntegerBoundedSearchList');
            $currentResultIds = extractIntegerBoundedSearchList('AuditStepType',
                    'LastUpdate',
        ...
        if ((isset($this->__filters['deletedInValues']))
            ...
            $currentResultIds = extractIntegerSearchList('AuditStepType',
                    'Deleted',
        ...
        if ((isset($this->__filters['nameInValues']))
            ...
            $currentResultIds = extractStringSearchList('AuditStepType',
                    'name',
        ...
        for ($i = 0; $i < $resultCount; $i++) {
            $object = new AuditStepType(intval($resultIdKeys[$i]), true);
    ...
    private function getSQLQueryString() {
        ...
        $selectionSQL = 'SELECT * FROM `auditsteptypetable` ';
        $countSQL = 'SELECT COUNT(*) FROM `auditsteptypetable` ';
        $criteriaSQL = '';
        $sortOrderSQL = '';
