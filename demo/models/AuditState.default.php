...
/**
 * CLASS AUDITSTATE
 * Implements AuditState Class properties and methods and
 * handles AuditState Class database transactions.	
 */
...
class AuditState {
    ...
    public function install() {
        ...
        // Update MySQL Table
        $SQLText = 'SHOW TABLES LIKE "auditstatetable"';
        ...
        if ($result->num_rows > 0) {
            ...
            $backupTableName = ('bck_auditstatetable' . date('YmdHis'));
            $SQLText = 'CREATE TABLE `'
                    . $backupTableName
                    . '` LIKE `auditstatetable`;';
            $this->__mySQLConnection->query($SQLText);
            $SQLText = 'INSERT `'
                    . $backupTableName
                    . '` SELECT * FROM `auditstatetable`;';
            $this->__mySQLConnection->query($SQLText);

        } else {

            // Create Table If Not Exists
            $SQLText = 'CREATE TABLE `auditstatetable` (
		...
		// enabled
		$strSQL = 'SHOW COLUMNS FROM `auditstatetable`'
				. ' LIKE "enabled";';
		$result = $this->__mySQLConnection->query($strSQL);
		$strSQL = 'ALTER TABLE `auditstatetable`'
				. (($result) ? ' ADD ' : ' MODIFY ')
	...
	/**
     * assign - Copies a AuditState instance to this instance.
     *
     * @param objAuditState [AuditState][in]: AuditState instance to be copied
     */
     ...
     *
     * @return Returns newly created AuditState id on success, false on failure.
     */
	public function insert() {
		...
		$SQLText = 'INSERT INTO `auditstatetable` '
				. '(`deleted`,'
				. '`creationdate`,'
				. '`lastupdate`'
				. ', `enabled`'
	...
	public function update() {
		...
		$SQLText = 'UPDATE `auditstatetable` SET '
				. '`deleted`={{deleted}},'
				. '`lastupdate`=NOW() '
				. ', `enabled`=\'{{parameter0}}\' '
				. ', `index`=\'{{parameter1}}\' '
	...
	public function revert($fromCache = false) {
    	...
    	if ($fromCache && ($this->id > 0)) {
			...
			$SQLText = 'SELECT * FROM `auditstatetable` WHERE `id`={{id}};';
			$SQLText = str_replace('{{id}}', intval($this->id), $SQLText);
	...
	public function delete() {
		...
		if ($this->deleted) {
			$SQLText = 'DELETE FROM `auditstatetable` '
					. ' WHERE `id`={{id}};';
		} else {
			$SQLText = 'UPDATE `auditstatetable` SET '
					. '`deleted`=1'
	...
	public function read() {
		...
		$cacheFile = (DBDIR . '/AuditState/' . intval($this->id) . '.php');
	...
	public function write() {
	    ...
	    if (0 == $this->id) {
	        ...
	        if (file_exists(DBDIR . '/AuditState/__id')) {
	            $this->id = file_get_contents(DBDIR . '/AuditState/__id');
	        } // if (file_exists(DBDIR . '/AuditState/__id')) {
	        ...
	        while(file_exists(DBDIR . '/AuditState/' . $this->id . '.php')
	                || file_exists(DBDIR . '/AuditState/--' . $this->id . '.php')) {
	            $this->id++;
	        } // while(file_exists(DBDIR . '/AuditState/' . $this->id . '.php')
	     
	        includeLibrary('writeStringToFileViaFTP');
	        writeStringToFileViaFTP('Database/AuditState/__id', $this->id);	     
	    } // if (0 == $this->id) {
        ...
        $cacheFile = ('Database/AuditState/' . $this->id . '.php');
	...
	public function remove() {
		...
		$cacheFile = (FTP_PRIMARY_HOME . '/Database/AuditState/' . $this->id . '.php');
		$newCacheFile = (FTP_PRIMARY_HOME . '/Database/AuditState/--' . $this->id . '.php');
	...
	public function cache() {
		...
		if (!$this->deleted) {
			...
			if (file_exists(DIR . '/events/onAuditStateCache.php')) {
				require_once(DIR . '/events/onAuditStateCache.php');
				onAuditStateCache($this, $this->id, $propertyValues, $bulk);
			} // if (file_exists(DIR . '/events/onAuditStateCache.php')) {
			...
			if (file_exists(DIR . '/events/onAuditStateUncache.php')) {
				require_once(DIR . '/events/onAuditStateUncache.php');
				onAuditStateUncache($this, $this->id, $bulk);
			} // if (file_exists(DIR . '/events/onAuditStateUncache.php')) {
    ...
    /**
     * find - Finds AuditState instances specified with the listing
     * criteria
     *
     * @return Returns true on success, false on failure.
     */
	...
	public function doBeforeInsert() {
		...
		if (file_exists(DIR . '/events/onBeforeAuditStateInsert.php')) {

			require_once(DIR . '/events/onBeforeAuditStateInsert.php');
			onBeforeAuditStateInsert($this);
	...
	public function doAfterInsert() {
		...
		if (file_exists(DIR . '/events/onAfterAuditStateInsert.php')) {

			require_once(DIR . '/events/onAfterAuditStateInsert.php');
			onAfterAuditStateInsert($this);
	...
	public function doBeforeUpdate() {
		...
		if (file_exists(DIR . '/events/onBeforeAuditStateUpdate.php')) {

			require_once(DIR . '/events/onBeforeAuditStateUpdate.php');
			onBeforeAuditStateUpdate($this);
	...
	public function doAfterUpdate() {
		...
		if (file_exists(DIR . '/events/onAfterAuditStateUpdate.php')) {

			require_once(DIR . '/events/onAfterAuditStateUpdate.php');
			onAfterAuditStateUpdate($this);
	...
	public function doBeforeDelete() {
		...
		if (file_exists(DIR . '/events/onBeforeAuditStateDelete.php')) {

			require_once(DIR . '/events/onBeforeAuditStateDelete.php');
			onBeforeAuditStateDelete($this);
	...
	public function doAfterDelete() {
		...
		if (file_exists(DIR . '/events/onAfterAuditStateDelete.php')) {

			require_once(DIR . '/events/onAfterAuditStateDelete.php');
			onAfterAuditStateDelete($this);
	...
	private function writeFilterCache() {
		...
		$success = writeIntegerFilterCache('AuditState',
				$this->id,
				'deleted',
				$this->deleted);
	    if (!$success) {
		...
		$success = writeStringFilterCache('AuditState',
				$this->id,
				'name',
				$this->name);
	    if (!$success) {
	...
	private function removeFilterCache($removeOriginalFilters = true) {
        ...
        if ($removeOriginalFilters) {
		    $cacheFile = (DBDIR . '/AuditState/' . $this->id . '.php');
	    ...
	    $success = removeIntegerFilterCache('AuditState',
				$current->id,
				'deleted',
				$current->deleted);
	    if (!$success) {
		...
		$success = removeStringFilterCache('AuditState',
				$current->id,
				'name',
				$current->name);	
	    if (!$success) {
    ...
    /**
     * generateListFromFile - Generates list of AuditState instances from file
    ...
    private function generateListFromFile() {
        ...
        if (($this->__filters['lastUpdateMinExclusive'] > 0)
            ...
            includeLibrary('extractIntegerBoundedSearchList');
            $currentResultIds = extractIntegerBoundedSearchList('AuditState',
                    'LastUpdate',
        ...
        if ((isset($this->__filters['deletedInValues']))
            ...
            $currentResultIds = extractIntegerSearchList('AuditState',
                    'Deleted',
        ...
        if ((isset($this->__filters['nameInValues']))
            ...
            $currentResultIds = extractStringSearchList('AuditState',
                    'name',
        ...
        for ($i = 0; $i < $resultCount; $i++) {
            $object = new AuditState(intval($resultIdKeys[$i]), true);
    ...
    private function getSQLQueryString() {
        ...
        $selectionSQL = 'SELECT * FROM `auditstatetable` ';
        $countSQL = 'SELECT COUNT(*) FROM `auditstatetable` ';
        $criteriaSQL = '';
        $sortOrderSQL = '';
