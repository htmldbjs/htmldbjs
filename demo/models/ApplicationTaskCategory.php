<?php
/**
 * CLASS APPLICATIONTASKCATEGORY
 * Implements ApplicationTaskCategory Class properties and methods and
 * handles ApplicationTaskCategory Class database transactions.	
 */
...
class ApplicationTaskCategory {
	...
	/**
     * ApplicationTaskCategory Constructor
     */
    ...
    public function install() {
        ...
        // Update MySQL Table
        $SQLText = 'SHOW TABLES LIKE "applicationtaskcategorytable"';
        ...
        if ($result->num_rows > 0) {
            ...
            $backupTableName = ('bck_applicationtaskcategorytable' . date('YmdHis'));
            $SQLText = 'CREATE TABLE `'
                    . $backupTableName
                    . '` LIKE `applicationtaskcategorytable`;';
            $this->__mySQLConnection->query($SQLText);
            $SQLText = 'INSERT `'
                    . $backupTableName
                    . '` SELECT * FROM `applicationtaskcategorytable`;';
            $this->__mySQLConnection->query($SQLText);

        } else {

            // Create Table If Not Exists
            $SQLText = 'CREATE TABLE `applicationtaskcategorytable` (
		...
		// enabled
		$strSQL = 'SHOW COLUMNS FROM `applicationtaskcategorytable`'
				. ' LIKE "enabled";';
		$result = $this->__mySQLConnection->query($strSQL);
		$strSQL = 'ALTER TABLE `applicationtaskcategorytable`'
				. (($result) ? ' ADD ' : ' MODIFY ')
	...
	/**
     * assign - Copies a ApplicationTaskCategory instance to this instance.
     *
     * @param objApplicationTaskCategory [ApplicationTaskCategory][in]: ApplicationTaskCategory instance to be copied
     */
     ...
     *
     * @return Returns newly created ApplicationTaskCategory id on success, false on failure.
     */
	public function insert() {
		...
		$SQLText = 'INSERT INTO `applicationtaskcategorytable` '
				. '(`deleted`,'
				. '`creationdate`,'
				. '`lastupdate`'
				. ', `enabled`'
		...
		$SQLText = str_replace('{{parameter1}}', intval($this->index), $SQLText);
	...
	public function update() {
		...
		$SQLText = 'UPDATE `applicationtaskcategorytable` SET '
				. '`deleted`={{deleted}},'
				. '`lastupdate`=NOW() '
				. ', `enabled`=\'{{parameter0}}\' '
				. ', `index`=\'{{parameter1}}\' '
	...
	public function revert($fromCache = false) {
    	...
    	if ($fromCache && ($this->id > 0)) {
			...
			$SQLText = 'SELECT * FROM `applicationtaskcategorytable` WHERE `id`={{id}};';
			$SQLText = str_replace('{{id}}', intval($this->id), $SQLText);
	...
	public function delete() {
		...
		if ($this->deleted) {
			$SQLText = 'DELETE FROM `applicationtaskcategorytable` '
					. ' WHERE `id`={{id}};';
		} else {
			$SQLText = 'UPDATE `applicationtaskcategorytable` SET '
					. '`deleted`=1'
	...
	public function read() {
		...
		$cacheFile = (DBDIR . '/ApplicationTaskCategory/' . intval($this->id) . '.php');
	...
	public function write() {
	    ...
	    if (0 == $this->id) {
	        ...
	        if (file_exists(DBDIR . '/ApplicationTaskCategory/__id')) {
	            $this->id = file_get_contents(DBDIR . '/ApplicationTaskCategory/__id');
	        } // if (file_exists(DBDIR . '/ApplicationTaskCategory/__id')) {
	        ...
	        while(file_exists(DBDIR . '/ApplicationTaskCategory/' . $this->id . '.php')
	                || file_exists(DBDIR . '/ApplicationTaskCategory/--' . $this->id . '.php')) {
	            $this->id++;
	        } // while(file_exists(DBDIR . '/ApplicationTaskCategory/' . $this->id . '.php')
	     
	        includeLibrary('writeStringToFileViaFTP');
	        writeStringToFileViaFTP('Database/ApplicationTaskCategory/__id', $this->id);	     
	    } // if (0 == $this->id) {
	    ...
	    $content = '<' . '?' . 'php '
	            ...
	            . '$' . 'object=new ApplicationTaskCategory;'
	            . '$' . 'object->id=' . $this->id . ';'
        ...
        $cacheFile = ('Database/ApplicationTaskCategory/' . $this->id . '.php');
	...
	public function remove() {
		...
		$cacheFile = (FTP_PRIMARY_HOME . '/Database/ApplicationTaskCategory/' . $this->id . '.php');
		$newCacheFile = (FTP_PRIMARY_HOME . '/Database/ApplicationTaskCategory/--' . $this->id . '.php');
	...
	public function cache() {
		...
		if (!$this->deleted) {
			...
			$propertyValues['AuditType/index'] = $this->get('AuditType/index');
			$propertyValues['AuditType/enabled'] = $this->get('AuditType/enabled');
			$propertyValues['AuditType/name'] = $this->get('AuditType/name'); 
			cacheClassProperties(__CLASS__, $this->id, $propertyValues, $bulk);

			if (file_exists(DIR . '/events/onApplicationTaskCategoryCache.php')) {
				require_once(DIR . '/events/onApplicationTaskCategoryCache.php');
				onApplicationTaskCategoryCache($this, $this->id, $propertyValues, $bulk);
			} // if (file_exists(DIR . '/events/onApplicationTaskCategoryCache.php')) {
			...
			if (file_exists(DIR . '/events/onApplicationTaskCategoryUncache.php')) {
				require_once(DIR . '/events/onApplicationTaskCategoryUncache.php');
				onApplicationTaskCategoryUncache($this, $this->id, $bulk);
			} // if (file_exists(DIR . '/events/onApplicationTaskCategoryUncache.php')) {
    ...
    /**
     * find - Finds ApplicationTaskCategory instances specified with the listing
     * criteria
     *
     * @return Returns true on success, false on failure.
     */
	...
	public function doBeforeInsert() {
		...
		if (file_exists(DIR . '/events/onBeforeApplicationTaskCategoryInsert.php')) {

			require_once(DIR . '/events/onBeforeApplicationTaskCategoryInsert.php');
			onBeforeApplicationTaskCategoryInsert($this);
	...
	public function doAfterInsert() {
		...
		if (file_exists(DIR . '/events/onAfterApplicationTaskCategoryInsert.php')) {

			require_once(DIR . '/events/onAfterApplicationTaskCategoryInsert.php');
			onAfterApplicationTaskCategoryInsert($this);
	...
	public function doBeforeUpdate() {
		...
		if (file_exists(DIR . '/events/onBeforeApplicationTaskCategoryUpdate.php')) {

			require_once(DIR . '/events/onBeforeApplicationTaskCategoryUpdate.php');
			onBeforeApplicationTaskCategoryUpdate($this);
	...
	public function doAfterUpdate() {
		...
		if (file_exists(DIR . '/events/onAfterApplicationTaskCategoryUpdate.php')) {

			require_once(DIR . '/events/onAfterApplicationTaskCategoryUpdate.php');
			onAfterApplicationTaskCategoryUpdate($this);
	...
	public function doBeforeDelete() {
		...
		if (file_exists(DIR . '/events/onBeforeApplicationTaskCategoryDelete.php')) {

			require_once(DIR . '/events/onBeforeApplicationTaskCategoryDelete.php');
			onBeforeApplicationTaskCategoryDelete($this);
	...
	public function doAfterDelete() {
		...
		if (file_exists(DIR . '/events/onAfterApplicationTaskCategoryDelete.php')) {

			require_once(DIR . '/events/onAfterApplicationTaskCategoryDelete.php');
			onAfterApplicationTaskCategoryDelete($this);
	...
	private function writeFilterCache() {
		...
		$success = writeIntegerFilterCache('ApplicationTaskCategory',
				$this->id,
				'deleted',
				$this->deleted);
	    if (!$success) {
		...
		$success = writeStringFilterCache('ApplicationTaskCategory',
				$this->id,
				'name',
				$this->name);
	    if (!$success) {
	...
	private function removeFilterCache($removeOriginalFilters = true) {
        ...
        if ($removeOriginalFilters) {
		    $cacheFile = (DBDIR . '/ApplicationTaskCategory/' . $this->id . '.php');
	    ...
	    $success = removeIntegerFilterCache('ApplicationTaskCategory',
				$current->id,
				'deleted',
				$current->deleted);
	    if (!$success) {
		...
		$success = removeStringFilterCache('ApplicationTaskCategory',
				$current->id,
				'name',
				$current->name);	
	    if (!$success) {
	...
	private function extractColumnValues() {
		...
		$this->__columnValues[$index] = $this->id;
				$expressionV0 = $this->getDisplayText('AuditType/index');

		$index++;
		$this->__columnValues[$index] = $expressionV0;

		$expressionV0 = $this->getDisplayText('AuditType/enabled');

		$index++;
		$this->__columnValues[$index] = $expressionV0;

		$expressionV0 = $this->getDisplayText('AuditType/name');
    ...
    /**
     * generateListFromFile - Generates list of ApplicationTaskCategory instances from file
    ...
    private function generateListFromFile() {
        ...
        if (($this->__filters['lastUpdateMinExclusive'] > 0)
            ...
            includeLibrary('extractIntegerBoundedSearchList');
            $currentResultIds = extractIntegerBoundedSearchList('ApplicationTaskCategory',
                    'LastUpdate',
        ...
        if ((isset($this->__filters['deletedInValues']))
            ...
            $currentResultIds = extractIntegerSearchList('ApplicationTaskCategory',
                    'Deleted',
        ...
        if ((isset($this->__filters['nameInValues']))
            ...
            $currentResultIds = extractStringSearchList('ApplicationTaskCategory',
                    'name',
        ...
        for ($i = 0; $i < $resultCount; $i++) {
            $object = new ApplicationTaskCategory(intval($resultIdKeys[$i]), true);
    ...
    private function getSQLQueryString() {
        ...
        $selectionSQL = 'SELECT * FROM `applicationtaskcategorytable` ';
        $countSQL = 'SELECT COUNT(*) FROM `applicationtaskcategorytable` ';
        $criteriaSQL = '';
        $sortOrderSQL = '';
