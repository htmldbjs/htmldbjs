...
/**
 * CLASS AUDITTYPE
 * Implements AuditType Class properties and methods and
 * handles AuditType Class database transactions.	
 */
...
class AuditType {
    ...
    public function install() {
        ...
        // Update MySQL Table
        $SQLText = 'SHOW TABLES LIKE "audittypetable"';
        ...
        if ($result->num_rows > 0) {
            ...
            $backupTableName = ('bck_audittypetable' . date('YmdHis'));
            ...
            $SQLText = 'INSERT `'
                    . $backupTableName
                    . '` SELECT * FROM `audittypetable`;';
            $this->__mySQLConnection->query($SQLText);

        } else {

            // Create Table If Not Exists
            $SQLText = 'CREATE TABLE `audittypetable` (
		...
		// enabled
		$strSQL = 'SHOW COLUMNS FROM `audittypetable`'
				. ' LIKE "enabled";';
		$result = $this->__mySQLConnection->query($strSQL);
		$strSQL = 'ALTER TABLE `audittypetable`'
				. (($result) ? ' ADD ' : ' MODIFY ')
	...
	/**
     * assign - Copies a AuditType instance to this instance.
     *
     * @param objAuditType [AuditType][in]: AuditType instance to be copied
     */
     ...
     *
     * @return Returns newly created AuditType id on success, false on failure.
     */
	public function insert() {
		...
		$SQLText = 'INSERT INTO `audittypetable` '
				. '(`deleted`,'
				. '`creationdate`,'
				. '`lastupdate`'
				. ', `enabled`'
	...
	public function update() {
		...
		$SQLText = 'UPDATE `audittypetable` SET '
				. '`deleted`={{deleted}},'
				. '`lastupdate`=NOW() '
				. ', `enabled`=\'{{parameter0}}\' '
				. ', `index`=\'{{parameter1}}\' '
	...
	public function revert($fromCache = false) {
    	...
    	if ($fromCache && ($this->id > 0)) {
			...
			$SQLText = 'SELECT * FROM `audittypetable` WHERE `id`={{id}};';
			$SQLText = str_replace('{{id}}', intval($this->id), $SQLText);
	...
	public function delete() {
		...
		if ($this->deleted) {
			$SQLText = 'DELETE FROM `audittypetable` '
					. ' WHERE `id`={{id}};';
		} else {
			$SQLText = 'UPDATE `audittypetable` SET '
					. '`deleted`=1'
	...
	public function read() {
		...
		$cacheFile = (DBDIR . '/AuditType/' . intval($this->id) . '.php');
	...
	public function write() {
	    ...
	    if (0 == $this->id) {
	        ...
	        if (file_exists(DBDIR . '/AuditType/__id')) {
	            $this->id = file_get_contents(DBDIR . '/AuditType/__id');
	        } // if (file_exists(DBDIR . '/AuditType/__id')) {
	        ...
	        while(file_exists(DBDIR . '/AuditType/' . $this->id . '.php')
	                || file_exists(DBDIR . '/AuditType/--' . $this->id . '.php')) {
	            $this->id++;
	        } // while(file_exists(DBDIR . '/AuditType/' . $this->id . '.php')
	     
	        includeLibrary('writeStringToFileViaFTP');
	        writeStringToFileViaFTP('Database/AuditType/__id', $this->id);	     
	    } // if (0 == $this->id) {
        ...
        $cacheFile = ('Database/AuditType/' . $this->id . '.php');
	...
	public function remove() {
		...
		$cacheFile = (FTP_PRIMARY_HOME . '/Database/AuditType/' . $this->id . '.php');
		$newCacheFile = (FTP_PRIMARY_HOME . '/Database/AuditType/--' . $this->id . '.php');
	...
	public function cache() {
		...
		if (!$this->deleted) {
			...
			if (file_exists(DIR . '/events/onAuditTypeCache.php')) {
				require_once(DIR . '/events/onAuditTypeCache.php');
				onAuditTypeCache($this, $this->id, $propertyValues, $bulk);
			} // if (file_exists(DIR . '/events/onAuditTypeCache.php')) {
			...
			if (file_exists(DIR . '/events/onAuditTypeUncache.php')) {
				require_once(DIR . '/events/onAuditTypeUncache.php');
				onAuditTypeUncache($this, $this->id, $bulk);
			} // if (file_exists(DIR . '/events/onAuditTypeUncache.php')) {
    ...
    /**
     * find - Finds AuditType instances specified with the listing
     * criteria
     *
     * @return Returns true on success, false on failure.
     */
	...
	public function doBeforeInsert() {
		...
		if (file_exists(DIR . '/events/onBeforeAuditTypeInsert.php')) {

			require_once(DIR . '/events/onBeforeAuditTypeInsert.php');
			onBeforeAuditTypeInsert($this);
	...
	public function doAfterInsert() {
		...
		if (file_exists(DIR . '/events/onAfterAuditTypeInsert.php')) {

			require_once(DIR . '/events/onAfterAuditTypeInsert.php');
			onAfterAuditTypeInsert($this);
	...
	public function doBeforeUpdate() {
		...
		if (file_exists(DIR . '/events/onBeforeAuditTypeUpdate.php')) {

			require_once(DIR . '/events/onBeforeAuditTypeUpdate.php');
			onBeforeAuditTypeUpdate($this);
	...
	public function doAfterUpdate() {
		...
		if (file_exists(DIR . '/events/onAfterAuditTypeUpdate.php')) {

			require_once(DIR . '/events/onAfterAuditTypeUpdate.php');
			onAfterAuditTypeUpdate($this);
	...
	public function doBeforeDelete() {
		...
		if (file_exists(DIR . '/events/onBeforeAuditTypeDelete.php')) {

			require_once(DIR . '/events/onBeforeAuditTypeDelete.php');
			onBeforeAuditTypeDelete($this);
	...
	public function doAfterDelete() {
		...
		if (file_exists(DIR . '/events/onAfterAuditTypeDelete.php')) {

			require_once(DIR . '/events/onAfterAuditTypeDelete.php');
			onAfterAuditTypeDelete($this);
	...
	private function writeFilterCache() {
		...
		$success = writeIntegerFilterCache('AuditType',
				$this->id,
				'deleted',
				$this->deleted);
	    if (!$success) {
		...
		$success = writeStringFilterCache('AuditType',
				$this->id,
				'name',
				$this->name);
	    if (!$success) {
	...
	private function removeFilterCache($removeOriginalFilters = true) {
        ...
        if ($removeOriginalFilters) {
		    $cacheFile = (DBDIR . '/AuditType/' . $this->id . '.php');
	    ...
	    $success = removeIntegerFilterCache('AuditType',
				$current->id,
				'deleted',
				$current->deleted);
	    if (!$success) {
		...
		$success = removeStringFilterCache('AuditType',
				$current->id,
				'name',
				$current->name);	
	    if (!$success) {
    ...
    /**
     * generateListFromFile - Generates list of AuditType instances from file
    ...
    private function generateListFromFile() {
        ...
        if (($this->__filters['lastUpdateMinExclusive'] > 0)
            ...
            includeLibrary('extractIntegerBoundedSearchList');
            $currentResultIds = extractIntegerBoundedSearchList('AuditType',
                    'LastUpdate',
        ...
        if ((isset($this->__filters['deletedInValues']))
            ...
            $currentResultIds = extractIntegerSearchList('AuditType',
                    'Deleted',
        ...
        if ((isset($this->__filters['nameInValues']))
            ...
            $currentResultIds = extractStringSearchList('AuditType',
                    'name',
        ...
        for ($i = 0; $i < $resultCount; $i++) {
            $object = new AuditType(intval($resultIdKeys[$i]), true);
    ...
    private function getSQLQueryString() {
        ...
        $selectionSQL = 'SELECT * FROM `audittypetable` ';
        $countSQL = 'SELECT COUNT(*) FROM `audittypetable` ';
        $criteriaSQL = '';
        $sortOrderSQL = '';
