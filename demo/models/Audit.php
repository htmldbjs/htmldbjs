...
class Audit {
	...
	public function request($requests = NULL, $prefix = '') {
		...
		$this->score = isset($requests[$prefix . 'score'])
				? floatval($requests[$prefix . 'score'])
				: $this->score;
	...
	public function insert() {
		...
		$SQLText = str_replace('{{parameter5}}', floatval($this->score), $SQLText);
		$SQLText = str_replace('{{parameter6}}',
				addslashes(
				$this->__mySQLConnection->real_escape_string(
				$this->notes)),
				$SQLText);
		$SQLText = str_replace('{{parameter7}}', intval($this->created_by), $SQLText);
	...
	public function write() {
	    ...
	    $content = '<' . '?' . 'php '
                ...
                . '$' . 'object->score=' . floatval($this->score) . ';'
	...
	private function writeFilterCache() {
		...
		$success = writeNumberFilterCache('Audit',
				$this->id,
				'score',
				$this->score);
	    if (!$success) {
	...
	private function removeFilterCache($removeOriginalFilters = true) {
		...
		$success = removeNumberFilterCache('Audit',
				$current->id,
				'score',
				$current->score);	
	    if (!$success) {
    ...
    private function generateListFromSQL() {
        ...
        while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
			...
			$object->score = floatval($row['score']);
    ...
    private function generateListFromFile() {
        ...
        if ((isset($this->__filters['scoreInValues']))
            ...
            includeLibrary('extractNumberSearchList');
            $currentResultIds = extractNumberSearchList('Audit',
                    'score',
            ...
            includeLibrary('extractNumberBoundedSearchList');
            $currentResultIds = extractNumberBoundedSearchList('Audit',
                    'score',
    ...
    private function getSQLQueryString() {
        ...
        $criteriaSQL = generateIntegerSQLCriteria(
        		...
        		$this->__mySQLConnection,
        		$criteriaSQL);
        $criteriaSQL = generateFloatSQLCriteria(
        		'score',
        		$this->__filters,
        		$this->__mySQLConnection,
        		$criteriaSQL);
