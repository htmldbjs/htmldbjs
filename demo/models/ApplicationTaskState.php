<?php
/**
 * CLASS APPLICATIONTASKSTATE
 * Implements ApplicationTaskState Class properties and methods and
 * handles ApplicationTaskState Class database transactions.	
 */
...
class ApplicationTaskState {
	...
	/**
     * ApplicationTaskState Constructor
     */
    ...
    public function install() {
        ...
        // Update MySQL Table
        $SQLText = 'SHOW TABLES LIKE "applicationtaskstatetable"';
        ...
        if ($result->num_rows > 0) {
            ...
            $backupTableName = ('bck_applicationtaskstatetable' . date('YmdHis'));
            $SQLText = 'CREATE TABLE `'
                    . $backupTableName
                    . '` LIKE `applicationtaskstatetable`;';
            $this->__mySQLConnection->query($SQLText);
            $SQLText = 'INSERT `'
                    . $backupTableName
                    . '` SELECT * FROM `applicationtaskstatetable`;';
            $this->__mySQLConnection->query($SQLText);

        } else {

            // Create Table If Not Exists
            $SQLText = 'CREATE TABLE `applicationtaskstatetable` (
		...
		// enabled
		$strSQL = 'SHOW COLUMNS FROM `applicationtaskstatetable`'
				. ' LIKE "enabled";';
		$result = $this->__mySQLConnection->query($strSQL);
		$strSQL = 'ALTER TABLE `applicationtaskstatetable`'
				. (($result) ? ' ADD ' : ' MODIFY ')
	...
	/**
     * assign - Copies a ApplicationTaskState instance to this instance.
     *
     * @param objApplicationTaskState [ApplicationTaskState][in]: ApplicationTaskState instance to be copied
     */
     ...
     *
     * @return Returns newly created ApplicationTaskState id on success, false on failure.
     */
	public function insert() {
		...
		$SQLText = 'INSERT INTO `applicationtaskstatetable` '
				. '(`deleted`,'
				. '`creationdate`,'
				. '`lastupdate`'
				. ', `enabled`'
	...
	public function update() {
		...
		$SQLText = 'UPDATE `applicationtaskstatetable` SET '
				. '`deleted`={{deleted}},'
				. '`lastupdate`=NOW() '
				. ', `enabled`=\'{{parameter0}}\' '
				. ', `index`=\'{{parameter1}}\' '
	...
	public function revert($fromCache = false) {
    	...
    	if ($fromCache && ($this->id > 0)) {
			...
			$SQLText = 'SELECT * FROM `applicationtaskstatetable` WHERE `id`={{id}};';
			$SQLText = str_replace('{{id}}', intval($this->id), $SQLText);
	...
	public function delete() {
		...
		if ($this->deleted) {
			$SQLText = 'DELETE FROM `applicationtaskstatetable` '
					. ' WHERE `id`={{id}};';
		} else {
			$SQLText = 'UPDATE `applicationtaskstatetable` SET '
					. '`deleted`=1'
	...
	public function read() {
		...
		$cacheFile = (DBDIR . '/ApplicationTaskState/' . intval($this->id) . '.php');
	...
	public function write() {
	    ...
	    if (0 == $this->id) {
	        ...
	        if (file_exists(DBDIR . '/ApplicationTaskState/__id')) {
	            $this->id = file_get_contents(DBDIR . '/ApplicationTaskState/__id');
	        } // if (file_exists(DBDIR . '/ApplicationTaskState/__id')) {
	        ...
	        while(file_exists(DBDIR . '/ApplicationTaskState/' . $this->id . '.php')
	                || file_exists(DBDIR . '/ApplicationTaskState/--' . $this->id . '.php')) {
	            $this->id++;
	        } // while(file_exists(DBDIR . '/ApplicationTaskState/' . $this->id . '.php')
	     
	        includeLibrary('writeStringToFileViaFTP');
	        writeStringToFileViaFTP('Database/ApplicationTaskState/__id', $this->id);	     
	    } // if (0 == $this->id) {
	    ...
	    $content = '<' . '?' . 'php '
	            ...
	            . '$' . 'object=new ApplicationTaskState;'
	            . '$' . 'object->id=' . $this->id . ';'
        ...
        $cacheFile = ('Database/ApplicationTaskState/' . $this->id . '.php');
	...
	public function remove() {
		...
		$cacheFile = (FTP_PRIMARY_HOME . '/Database/ApplicationTaskState/' . $this->id . '.php');
		$newCacheFile = (FTP_PRIMARY_HOME . '/Database/ApplicationTaskState/--' . $this->id . '.php');
	...
	public function cache() {
		...
		if (!$this->deleted) {
			...
			if (file_exists(DIR . '/events/onApplicationTaskStateCache.php')) {
				require_once(DIR . '/events/onApplicationTaskStateCache.php');
				onApplicationTaskStateCache($this, $this->id, $propertyValues, $bulk);
			} // if (file_exists(DIR . '/events/onApplicationTaskStateCache.php')) {
			...
			if (file_exists(DIR . '/events/onApplicationTaskStateUncache.php')) {
				require_once(DIR . '/events/onApplicationTaskStateUncache.php');
				onApplicationTaskStateUncache($this, $this->id, $bulk);
			} // if (file_exists(DIR . '/events/onApplicationTaskStateUncache.php')) {
    ...
    /**
     * find - Finds ApplicationTaskState instances specified with the listing
     * criteria
     *
     * @return Returns true on success, false on failure.
     */
	...
	public function doBeforeInsert() {
		...
		if (file_exists(DIR . '/events/onBeforeApplicationTaskStateInsert.php')) {

			require_once(DIR . '/events/onBeforeApplicationTaskStateInsert.php');
			onBeforeApplicationTaskStateInsert($this);
	...
	public function doAfterInsert() {
		...
		if (file_exists(DIR . '/events/onAfterApplicationTaskStateInsert.php')) {

			require_once(DIR . '/events/onAfterApplicationTaskStateInsert.php');
			onAfterApplicationTaskStateInsert($this);
	...
	public function doBeforeUpdate() {
		...
		if (file_exists(DIR . '/events/onBeforeApplicationTaskStateUpdate.php')) {

			require_once(DIR . '/events/onBeforeApplicationTaskStateUpdate.php');
			onBeforeApplicationTaskStateUpdate($this);
	...
	public function doAfterUpdate() {
		...
		if (file_exists(DIR . '/events/onAfterApplicationTaskStateUpdate.php')) {

			require_once(DIR . '/events/onAfterApplicationTaskStateUpdate.php');
			onAfterApplicationTaskStateUpdate($this);
	...
	public function doBeforeDelete() {
		...
		if (file_exists(DIR . '/events/onBeforeApplicationTaskStateDelete.php')) {

			require_once(DIR . '/events/onBeforeApplicationTaskStateDelete.php');
			onBeforeApplicationTaskStateDelete($this);
	...
	public function doAfterDelete() {
		...
		if (file_exists(DIR . '/events/onAfterApplicationTaskStateDelete.php')) {

			require_once(DIR . '/events/onAfterApplicationTaskStateDelete.php');
			onAfterApplicationTaskStateDelete($this);
	...
	private function writeFilterCache() {
		...
		$success = writeIntegerFilterCache('ApplicationTaskState',
				$this->id,
				'deleted',
				$this->deleted);
	    if (!$success) {
		...
		$success = writeStringFilterCache('ApplicationTaskState',
				$this->id,
				'name',
				$this->name);
	    if (!$success) {
	...
	private function removeFilterCache($removeOriginalFilters = true) {
        ...
        if ($removeOriginalFilters) {
		    $cacheFile = (DBDIR . '/ApplicationTaskState/' . $this->id . '.php');
	    ...
	    $success = removeIntegerFilterCache('ApplicationTaskState',
				$current->id,
				'deleted',
				$current->deleted);
	    if (!$success) {
		...
		$success = removeStringFilterCache('ApplicationTaskState',
				$current->id,
				'name',
				$current->name);	
	    if (!$success) {
    ...
    /**
     * generateListFromFile - Generates list of ApplicationTaskState instances from file
    ...
    private function generateListFromFile() {
        ...
        if (($this->__filters['lastUpdateMinExclusive'] > 0)
            ...
            includeLibrary('extractIntegerBoundedSearchList');
            $currentResultIds = extractIntegerBoundedSearchList('ApplicationTaskState',
                    'LastUpdate',
        ...
        if ((isset($this->__filters['deletedInValues']))
            ...
            $currentResultIds = extractIntegerSearchList('ApplicationTaskState',
                    'Deleted',
        ...
        if ((isset($this->__filters['nameInValues']))
            ...
            $currentResultIds = extractStringSearchList('ApplicationTaskState',
                    'name',
        ...
        for ($i = 0; $i < $resultCount; $i++) {
            $object = new ApplicationTaskState(intval($resultIdKeys[$i]), true);
    ...
    private function getSQLQueryString() {
        ...
        $selectionSQL = 'SELECT * FROM `applicationtaskstatetable` ';
        $countSQL = 'SELECT COUNT(*) FROM `applicationtaskstatetable` ';
        $criteriaSQL = '';
        $sortOrderSQL = '';
