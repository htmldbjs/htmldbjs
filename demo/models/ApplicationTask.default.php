 ...
 * Implements ApplicationTask Class properties and methods and
 * handles ApplicationTask Class database transactions.	
 */
...
class ApplicationTask {
    ...
    public function install() {
        ...
        // Update MySQL Table
        $SQLText = 'SHOW TABLES LIKE "applicationtasktable"';
        ...
        if ($result->num_rows > 0) {
            ...
            $backupTableName = ('bck_applicationtasktable' . date('YmdHis'));
            $SQLText = 'CREATE TABLE `'
                    . $backupTableName
                    . '` LIKE `applicationtasktable`;';
            $this->__mySQLConnection->query($SQLText);
            $SQLText = 'INSERT `'
                    . $backupTableName
                    . '` SELECT * FROM `applicationtasktable`;';
            $this->__mySQLConnection->query($SQLText);

        } else {

            // Create Table If Not Exists
            $SQLText = 'CREATE TABLE `applicationtasktable` (
		...
		// application_id
		$strSQL = 'SHOW COLUMNS FROM `applicationtasktable`'
				. ' LIKE "application_id";';
		$result = $this->__mySQLConnection->query($strSQL);
		$strSQL = 'ALTER TABLE `applicationtasktable`'
				. (($result) ? ' ADD ' : ' MODIFY ')
        ...
        // responsible
        $SQLText = 'SHOW TABLES LIKE "applicationtaskresponsibletable"';
        ...
        if ($result->num_rows > 0) {
            ...
            // Backup Old Table If Exits        
            $backupTableName = ('bck_applicationtaskresponsibletable' . date('YmdHis'));
            $SQLText = 'CREATE TABLE `'
                    . $backupTableName
                    . '` LIKE `applicationtaskresponsibletable`;';
            $this->__mySQLConnection->query($SQLText);
            $SQLText = 'INSERT `'
                    . $backupTableName
                    . '` SELECT * FROM `applicationtaskresponsibletable`;';
            $this->__mySQLConnection->query($SQLText);

        } else {

            // Create Table If Not Exists
            $SQLText = 'CREATE TABLE `applicationtaskresponsibletable` (
                `index` BIGINT UNSIGNED NULL,
                `applicationtask_id` BIGINT UNSIGNED NULL,
                `responsible` BIGINT UNSIGNED NULL,
                PRIMARY KEY  (`index`,`applicationtask_id`,`responsible`)) ENGINE=\'MyISAM\' ROW_FORMAT=FIXED;';
            $this->__mySQLConnection->query($SQLText);
	...
	/**
     * assign - Copies a ApplicationTask instance to this instance.
     *
     * @param objApplicationTask [ApplicationTask][in]: ApplicationTask instance to be copied
     */
     ...
     *
     * @return Returns newly created ApplicationTask id on success, false on failure.
     */
	public function insert() {
		...
		$SQLText = 'INSERT INTO `applicationtasktable` '
				. '(`deleted`,'
				. '`creationdate`,'
				. '`lastupdate`'
				. ', `application_id`'
		...
		$SQLTemplate = 'INSERT INTO `applicationtaskresponsibletable` VALUES({{parameter0}},{{parameter1}},{{parameter2}});';
		$selections = explode(',', $this->responsible);
		$selectionCount = count($selections);
		$SQLText = '';
	...
	public function update() {
		...
		$SQLText = 'UPDATE `applicationtasktable` SET '
				. '`deleted`={{deleted}},'
				. '`lastupdate`=NOW() '
		...
		$SQLText = ('DELETE FROM `applicationtaskresponsibletable` '
				. 'WHERE `applicationtask_id` = {{id}};');
		$SQLText = str_replace('{{id}}', intval($this->id), $SQLText);
		$this->__mySQLConnection->query($SQLText); 
	...
	public function revert($fromCache = false) {
    	...
    	if ($fromCache && ($this->id > 0)) {
			...
			$SQLText = 'SELECT * FROM `applicationtasktable` WHERE `id`={{id}};';
			$SQLText = str_replace('{{id}}', intval($this->id), $SQLText);
	...
	public function delete() {
		...
		if ($this->deleted) {
			$SQLText = 'DELETE FROM `applicationtasktable` '
					. ' WHERE `id`={{id}};';
		} else {
			$SQLText = 'UPDATE `applicationtasktable` SET '
					. '`deleted`=1'
	...
	public function read() {
		...
		$cacheFile = (DBDIR . '/ApplicationTask/' . intval($this->id) . '.php');
	...
	public function write() {
	    ...
	    if (0 == $this->id) {
	        ...
	        if (file_exists(DBDIR . '/ApplicationTask/__id')) {
	            $this->id = file_get_contents(DBDIR . '/ApplicationTask/__id');
	        } // if (file_exists(DBDIR . '/ApplicationTask/__id')) {
	        ...
	        while(file_exists(DBDIR . '/ApplicationTask/' . $this->id . '.php')
	                || file_exists(DBDIR . '/ApplicationTask/--' . $this->id . '.php')) {
	            $this->id++;
	        } // while(file_exists(DBDIR . '/ApplicationTask/' . $this->id . '.php')
	     
	        includeLibrary('writeStringToFileViaFTP');
	        writeStringToFileViaFTP('Database/ApplicationTask/__id', $this->id);	     
	    } // if (0 == $this->id) {
        ...
        $cacheFile = ('Database/ApplicationTask/' . $this->id . '.php');
	...
	public function remove() {
		...
		$cacheFile = (FTP_PRIMARY_HOME . '/Database/ApplicationTask/' . $this->id . '.php');
		$newCacheFile = (FTP_PRIMARY_HOME . '/Database/ApplicationTask/--' . $this->id . '.php');
	...
	public function cache() {
		...
		if (!$this->deleted) {
			...
			$propertyValues['ApplicationTask/application_id'] = $this->get('ApplicationTask/application_id');
			$propertyValues['ApplicationTask/application_task_code'] = $this->get('ApplicationTask/application_task_code');
			$propertyValues['ApplicationTask/application_task_category_id'] = $this->get('ApplicationTask/application_task_category_id');
			$propertyValues['ApplicationTask/description'] = $this->get('ApplicationTask/description');
			$propertyValues['ApplicationTask/application_task_state_id'] = $this->get('ApplicationTask/application_task_state_id'); 
			cacheClassProperties(__CLASS__, $this->id, $propertyValues, $bulk);

			if (file_exists(DIR . '/events/onApplicationTaskCache.php')) {
				require_once(DIR . '/events/onApplicationTaskCache.php');
				onApplicationTaskCache($this, $this->id, $propertyValues, $bulk);
			} // if (file_exists(DIR . '/events/onApplicationTaskCache.php')) {
			...
			if (file_exists(DIR . '/events/onApplicationTaskUncache.php')) {
				require_once(DIR . '/events/onApplicationTaskUncache.php');
				onApplicationTaskUncache($this, $this->id, $bulk);
			} // if (file_exists(DIR . '/events/onApplicationTaskUncache.php')) {
    ...
    /**
     * find - Finds ApplicationTask instances specified with the listing
     * criteria
     *
     * @return Returns true on success, false on failure.
     */
	...
	public function doBeforeInsert() {
		...
		if (file_exists(DIR . '/events/onBeforeApplicationTaskInsert.php')) {

			require_once(DIR . '/events/onBeforeApplicationTaskInsert.php');
			onBeforeApplicationTaskInsert($this);
	...
	public function doAfterInsert() {
		...
		if (file_exists(DIR . '/events/onAfterApplicationTaskInsert.php')) {

			require_once(DIR . '/events/onAfterApplicationTaskInsert.php');
			onAfterApplicationTaskInsert($this);
	...
	public function doBeforeUpdate() {
		...
		if (file_exists(DIR . '/events/onBeforeApplicationTaskUpdate.php')) {

			require_once(DIR . '/events/onBeforeApplicationTaskUpdate.php');
			onBeforeApplicationTaskUpdate($this);
	...
	public function doAfterUpdate() {
		...
		if (file_exists(DIR . '/events/onAfterApplicationTaskUpdate.php')) {

			require_once(DIR . '/events/onAfterApplicationTaskUpdate.php');
			onAfterApplicationTaskUpdate($this);
	...
	public function doBeforeDelete() {
		...
		if (file_exists(DIR . '/events/onBeforeApplicationTaskDelete.php')) {

			require_once(DIR . '/events/onBeforeApplicationTaskDelete.php');
			onBeforeApplicationTaskDelete($this);
	...
	public function doAfterDelete() {
		...
		if (file_exists(DIR . '/events/onAfterApplicationTaskDelete.php')) {

			require_once(DIR . '/events/onAfterApplicationTaskDelete.php');
			onAfterApplicationTaskDelete($this);
	...
	private function writeFilterCache() {
		...
		$success = writeIntegerFilterCache('ApplicationTask',
				$this->id,
				'deleted',
				$this->deleted);
	    if (!$success) {
		...
		$success = writeStringFilterCache('ApplicationTask',
				$this->id,
				'application_task_code',
				$this->application_task_code);
	    if (!$success) {
	...
	private function removeFilterCache($removeOriginalFilters = true) {
        ...
        if ($removeOriginalFilters) {
		    $cacheFile = (DBDIR . '/ApplicationTask/' . $this->id . '.php');
	    ...
	    $success = removeIntegerFilterCache('ApplicationTask',
				$current->id,
				'deleted',
				$current->deleted);
	    if (!$success) {
		...
		$success = removeStringFilterCache('ApplicationTask',
				$current->id,
				'application_task_code',
	...
	private function extractColumnValues() {
		...
		$this->__columnValues[$index] = $this->id;
				$expressionV0 = $this->getDisplayText('ApplicationTask/application_id');

		$index++;
		$this->__columnValues[$index] = $expressionV0;

		$expressionV0 = $this->getDisplayText('ApplicationTask/application_task_code');

		$index++;
		$this->__columnValues[$index] = $expressionV0;

		$expressionV0 = $this->getDisplayText('ApplicationTask/application_task_category_id');

		$index++;
		$this->__columnValues[$index] = $expressionV0;

		$expressionV0 = $this->getDisplayText('ApplicationTask/description');

		$index++;
		$this->__columnValues[$index] = $expressionV0;

		$expressionV0 = $this->getDisplayText('ApplicationTask/application_task_state_id');
    ...
    /**
     * generateListFromFile - Generates list of ApplicationTask instances from file
    ...
    private function generateListFromFile() {
        ...
        if (($this->__filters['lastUpdateMinExclusive'] > 0)
            ...
            includeLibrary('extractIntegerBoundedSearchList');
            $currentResultIds = extractIntegerBoundedSearchList('ApplicationTask',
                    'LastUpdate',
        ...
        if ((isset($this->__filters['deletedInValues']))
            ...
            $currentResultIds = extractIntegerSearchList('ApplicationTask',
                    'Deleted',
        ...
        if ((isset($this->__filters['application_task_codeInValues']))
            ...
            $currentResultIds = extractStringSearchList('ApplicationTask',
                    'application_task_code',
        ...
        for ($i = 0; $i < $resultCount; $i++) {
            $object = new ApplicationTask(intval($resultIdKeys[$i]), true);
    ...
    private function getSQLQueryString() {
        ...
        $selectionSQL = 'SELECT * FROM `applicationtasktable` ';
        $countSQL = 'SELECT COUNT(*) FROM `applicationtasktable` ';
        $criteriaSQL = '';
        $sortOrderSQL = '';
	...
	private function getMultipleSelectionCSV($propertyName, $parent_id = 0) {
		...
		$tableName = ('applicationtask' . strtolower($propertyName) . 'table');
		$SQLText = 'SELECT * FROM `'
				. $tableName
				. '` WHERE `applicationtask_id` = {{id}};';
