<?php
/**
 * CLASS AUDITSTEPCATEGORY
 * Implements AuditStepCategory Class properties and methods and
 * handles AuditStepCategory Class database transactions.	
 */
...
class AuditStepCategory {
    ...
    public function install() {
        ...
        // Update MySQL Table
        $SQLText = 'SHOW TABLES LIKE "auditstepcategorytable"';
        ...
        if ($result->num_rows > 0) {
            ...
            $backupTableName = ('bck_auditstepcategorytable' . date('YmdHis'));
            $SQLText = 'CREATE TABLE `'
                    . $backupTableName
                    . '` LIKE `auditstepcategorytable`;';
            $this->__mySQLConnection->query($SQLText);
            $SQLText = 'INSERT `'
                    . $backupTableName
                    . '` SELECT * FROM `auditstepcategorytable`;';
            $this->__mySQLConnection->query($SQLText);

        } else {

            // Create Table If Not Exists
            $SQLText = 'CREATE TABLE `auditstepcategorytable` (
		...
		// enabled
		$strSQL = 'SHOW COLUMNS FROM `auditstepcategorytable`'
				. ' LIKE "enabled";';
		$result = $this->__mySQLConnection->query($strSQL);
		$strSQL = 'ALTER TABLE `auditstepcategorytable`'
				. (($result) ? ' ADD ' : ' MODIFY ')
	...
	/**
     * assign - Copies a AuditStepCategory instance to this instance.
     *
     * @param objAuditStepCategory [AuditStepCategory][in]: AuditStepCategory instance to be copied
     */
     ...
     *
     * @return Returns newly created AuditStepCategory id on success, false on failure.
     */
	public function insert() {
		...
		$SQLText = 'INSERT INTO `auditstepcategorytable` '
				. '(`deleted`,'
				. '`creationdate`,'
				. '`lastupdate`'
				. ', `enabled`'
	...
	public function update() {
		...
		$SQLText = 'UPDATE `auditstepcategorytable` SET '
				. '`deleted`={{deleted}},'
				. '`lastupdate`=NOW() '
				. ', `enabled`=\'{{parameter0}}\' '
				. ', `index`=\'{{parameter1}}\' '
	...
	public function revert($fromCache = false) {
    	...
    	if ($fromCache && ($this->id > 0)) {
			...
			$SQLText = 'SELECT * FROM `auditstepcategorytable` WHERE `id`={{id}};';
			$SQLText = str_replace('{{id}}', intval($this->id), $SQLText);
	...
	public function delete() {
		...
		if ($this->deleted) {
			$SQLText = 'DELETE FROM `auditstepcategorytable` '
					. ' WHERE `id`={{id}};';
		} else {
			$SQLText = 'UPDATE `auditstepcategorytable` SET '
					. '`deleted`=1'
	...
	public function read() {
		...
		$cacheFile = (DBDIR . '/AuditStepCategory/' . intval($this->id) . '.php');
	...
	public function write() {
	    ...
	    if (0 == $this->id) {
	        ...
	        if (file_exists(DBDIR . '/AuditStepCategory/__id')) {
	            $this->id = file_get_contents(DBDIR . '/AuditStepCategory/__id');
	        } // if (file_exists(DBDIR . '/AuditStepCategory/__id')) {
	        ...
	        while(file_exists(DBDIR . '/AuditStepCategory/' . $this->id . '.php')
	                || file_exists(DBDIR . '/AuditStepCategory/--' . $this->id . '.php')) {
	            $this->id++;
	        } // while(file_exists(DBDIR . '/AuditStepCategory/' . $this->id . '.php')
	     
	        includeLibrary('writeStringToFileViaFTP');
	        writeStringToFileViaFTP('Database/AuditStepCategory/__id', $this->id);	     
	    } // if (0 == $this->id) {
        ...
        $cacheFile = ('Database/AuditStepCategory/' . $this->id . '.php');
	...
	public function remove() {
		...
		$cacheFile = (FTP_PRIMARY_HOME . '/Database/AuditStepCategory/' . $this->id . '.php');
		$newCacheFile = (FTP_PRIMARY_HOME . '/Database/AuditStepCategory/--' . $this->id . '.php');
	...
	public function cache() {
		...
		if (!$this->deleted) {
			...
			if (file_exists(DIR . '/events/onAuditStepCategoryCache.php')) {
				require_once(DIR . '/events/onAuditStepCategoryCache.php');
				onAuditStepCategoryCache($this, $this->id, $propertyValues, $bulk);
			} // if (file_exists(DIR . '/events/onAuditStepCategoryCache.php')) {
			...
			if (file_exists(DIR . '/events/onAuditStepCategoryUncache.php')) {
				require_once(DIR . '/events/onAuditStepCategoryUncache.php');
				onAuditStepCategoryUncache($this, $this->id, $bulk);
			} // if (file_exists(DIR . '/events/onAuditStepCategoryUncache.php')) {
    ...
    /**
     * find - Finds AuditStepCategory instances specified with the listing
     * criteria
     *
     * @return Returns true on success, false on failure.
     */
	...
	public function doBeforeInsert() {
		...
		if (file_exists(DIR . '/events/onBeforeAuditStepCategoryInsert.php')) {

			require_once(DIR . '/events/onBeforeAuditStepCategoryInsert.php');
			onBeforeAuditStepCategoryInsert($this);
	...
	public function doAfterInsert() {
		...
		if (file_exists(DIR . '/events/onAfterAuditStepCategoryInsert.php')) {

			require_once(DIR . '/events/onAfterAuditStepCategoryInsert.php');
			onAfterAuditStepCategoryInsert($this);
	...
	public function doBeforeUpdate() {
		...
		if (file_exists(DIR . '/events/onBeforeAuditStepCategoryUpdate.php')) {

			require_once(DIR . '/events/onBeforeAuditStepCategoryUpdate.php');
			onBeforeAuditStepCategoryUpdate($this);
	...
	public function doAfterUpdate() {
		...
		if (file_exists(DIR . '/events/onAfterAuditStepCategoryUpdate.php')) {

			require_once(DIR . '/events/onAfterAuditStepCategoryUpdate.php');
			onAfterAuditStepCategoryUpdate($this);
	...
	public function doBeforeDelete() {
		...
		if (file_exists(DIR . '/events/onBeforeAuditStepCategoryDelete.php')) {

			require_once(DIR . '/events/onBeforeAuditStepCategoryDelete.php');
			onBeforeAuditStepCategoryDelete($this);
	...
	public function doAfterDelete() {
		...
		if (file_exists(DIR . '/events/onAfterAuditStepCategoryDelete.php')) {

			require_once(DIR . '/events/onAfterAuditStepCategoryDelete.php');
			onAfterAuditStepCategoryDelete($this);
	...
	private function writeFilterCache() {
		...
		$success = writeIntegerFilterCache('AuditStepCategory',
				$this->id,
				'deleted',
				$this->deleted);
	    if (!$success) {
		...
		$success = writeStringFilterCache('AuditStepCategory',
				$this->id,
				'name',
				$this->name);
	    if (!$success) {
	...
	private function removeFilterCache($removeOriginalFilters = true) {
        ...
        if ($removeOriginalFilters) {
		    $cacheFile = (DBDIR . '/AuditStepCategory/' . $this->id . '.php');
	    ...
	    $success = removeIntegerFilterCache('AuditStepCategory',
				$current->id,
				'deleted',
				$current->deleted);
	    if (!$success) {
		...
		$success = removeStringFilterCache('AuditStepCategory',
				$current->id,
				'name',
				$current->name);	
	    if (!$success) {
    ...
    /**
     * generateListFromFile - Generates list of AuditStepCategory instances from file
    ...
    private function generateListFromFile() {
        ...
        if (($this->__filters['lastUpdateMinExclusive'] > 0)
            ...
            includeLibrary('extractIntegerBoundedSearchList');
            $currentResultIds = extractIntegerBoundedSearchList('AuditStepCategory',
                    'LastUpdate',
        ...
        if ((isset($this->__filters['deletedInValues']))
            ...
            $currentResultIds = extractIntegerSearchList('AuditStepCategory',
                    'Deleted',
        ...
        if ((isset($this->__filters['nameInValues']))
            ...
            $currentResultIds = extractStringSearchList('AuditStepCategory',
                    'name',
        ...
        for ($i = 0; $i < $resultCount; $i++) {
            $object = new AuditStepCategory(intval($resultIdKeys[$i]), true);
    ...
    private function getSQLQueryString() {
        ...
        $selectionSQL = 'SELECT * FROM `auditstepcategorytable` ';
        $countSQL = 'SELECT COUNT(*) FROM `auditstepcategorytable` ';
        $criteriaSQL = '';
        $sortOrderSQL = '';
