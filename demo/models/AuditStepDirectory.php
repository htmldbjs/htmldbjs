<?php
/**
 * CLASS AUDITSTEPDIRECTORY
 * Implements AuditStepDirectory Class properties and methods and
 * handles AuditStepDirectory Class database transactions.	
 */
...
class AuditStepDirectory {
	...
	/**
     * AuditStepDirectory Constructor
     */
    ...
    public function install() {
        ...
        // Update MySQL Table
        $SQLText = 'SHOW TABLES LIKE "auditstepdirectorytable"';
        ...
        if ($result->num_rows > 0) {
            ...
            $backupTableName = ('bck_auditstepdirectorytable' . date('YmdHis'));
            $SQLText = 'CREATE TABLE `'
                    . $backupTableName
                    . '` LIKE `auditstepdirectorytable`;';
            $this->__mySQLConnection->query($SQLText);
            $SQLText = 'INSERT `'
                    . $backupTableName
                    . '` SELECT * FROM `auditstepdirectorytable`;';
            $this->__mySQLConnection->query($SQLText);

        } else {

            // Create Table If Not Exists
            $SQLText = 'CREATE TABLE `auditstepdirectorytable` (
		...
		// audit_id
		$strSQL = 'SHOW COLUMNS FROM `auditstepdirectorytable`'
				. ' LIKE "audit_id";';
		$result = $this->__mySQLConnection->query($strSQL);
		$strSQL = 'ALTER TABLE `auditstepdirectorytable`'
				. (($result) ? ' ADD ' : ' MODIFY ')
	...
	/**
     * assign - Copies a AuditStepDirectory instance to this instance.
     *
     * @param objAuditStepDirectory [AuditStepDirectory][in]: AuditStepDirectory instance to be copied
     */
     ...
     *
     * @return Returns newly created AuditStepDirectory id on success, false on failure.
     */
	public function insert() {
		...
		$SQLText = 'INSERT INTO `auditstepdirectorytable` '
				. '(`deleted`,'
				. '`creationdate`,'
				. '`lastupdate`'
				. ', `audit_id`'
		...
		$SQLText = str_replace('{{parameter7}}', addslashes($this->photos), $SQLText);
	...
	public function update() {
		...
		$SQLText = 'UPDATE `auditstepdirectorytable` SET '
				. '`deleted`={{deleted}},'
				. '`lastupdate`=NOW() '
				. ', `audit_id`=\'{{parameter0}}\' '
	...
	public function revert($fromCache = false) {
    	...
    	if ($fromCache && ($this->id > 0)) {
			...
			$SQLText = 'SELECT * FROM `auditstepdirectorytable` WHERE `id`={{id}};';
			$SQLText = str_replace('{{id}}', intval($this->id), $SQLText);
	...
	public function delete() {
		...
		if ($this->deleted) {
			$SQLText = 'DELETE FROM `auditstepdirectorytable` '
					. ' WHERE `id`={{id}};';
		} else {
			$SQLText = 'UPDATE `auditstepdirectorytable` SET '
					. '`deleted`=1'
	...
	public function read() {
		...
		$cacheFile = (DBDIR . '/AuditStepDirectory/' . intval($this->id) . '.php');
	...
	public function write() {
	    ...
	    if (0 == $this->id) {
	        ...
	        if (file_exists(DBDIR . '/AuditStepDirectory/__id')) {
	            $this->id = file_get_contents(DBDIR . '/AuditStepDirectory/__id');
	        } // if (file_exists(DBDIR . '/AuditStepDirectory/__id')) {
	        ...
	        while(file_exists(DBDIR . '/AuditStepDirectory/' . $this->id . '.php')
	                || file_exists(DBDIR . '/AuditStepDirectory/--' . $this->id . '.php')) {
	            $this->id++;
	        } // while(file_exists(DBDIR . '/AuditStepDirectory/' . $this->id . '.php')
	     
	        includeLibrary('writeStringToFileViaFTP');
	        writeStringToFileViaFTP('Database/AuditStepDirectory/__id', $this->id);	     
	    } // if (0 == $this->id) {
        ...
        $cacheFile = ('Database/AuditStepDirectory/' . $this->id . '.php');
	...
	public function remove() {
		...
		$cacheFile = (FTP_PRIMARY_HOME . '/Database/AuditStepDirectory/' . $this->id . '.php');
		$newCacheFile = (FTP_PRIMARY_HOME . '/Database/AuditStepDirectory/--' . $this->id . '.php');
	...
	public function cache() {
		...
		if (!$this->deleted) {
			...
			$propertyValues['AuditStepDirectory/yes'] = $this->get('AuditStepDirectory/yes');
			$propertyValues['AuditStepDirectory/no'] = $this->get('AuditStepDirectory/no'); 
			cacheClassProperties(__CLASS__, $this->id, $propertyValues, $bulk);

			if (file_exists(DIR . '/events/onAuditStepDirectoryCache.php')) {
				require_once(DIR . '/events/onAuditStepDirectoryCache.php');
				onAuditStepDirectoryCache($this, $this->id, $propertyValues, $bulk);
			} // if (file_exists(DIR . '/events/onAuditStepDirectoryCache.php')) {
			...
			if (file_exists(DIR . '/events/onAuditStepDirectoryUncache.php')) {
				require_once(DIR . '/events/onAuditStepDirectoryUncache.php');
				onAuditStepDirectoryUncache($this, $this->id, $bulk);
			} // if (file_exists(DIR . '/events/onAuditStepDirectoryUncache.php')) {
    ...
    /**
     * find - Finds AuditStepDirectory instances specified with the listing
     * criteria
     *
     * @return Returns true on success, false on failure.
     */
	...
	public function doBeforeInsert() {
		...
		if (file_exists(DIR . '/events/onBeforeAuditStepDirectoryInsert.php')) {

			require_once(DIR . '/events/onBeforeAuditStepDirectoryInsert.php');
			onBeforeAuditStepDirectoryInsert($this);
	...
	public function doAfterInsert() {
		...
		if (file_exists(DIR . '/events/onAfterAuditStepDirectoryInsert.php')) {

			require_once(DIR . '/events/onAfterAuditStepDirectoryInsert.php');
			onAfterAuditStepDirectoryInsert($this);
	...
	public function doBeforeUpdate() {
		...
		if (file_exists(DIR . '/events/onBeforeAuditStepDirectoryUpdate.php')) {

			require_once(DIR . '/events/onBeforeAuditStepDirectoryUpdate.php');
			onBeforeAuditStepDirectoryUpdate($this);
	...
	public function doAfterUpdate() {
		...
		if (file_exists(DIR . '/events/onAfterAuditStepDirectoryUpdate.php')) {

			require_once(DIR . '/events/onAfterAuditStepDirectoryUpdate.php');
			onAfterAuditStepDirectoryUpdate($this);
	...
	public function doBeforeDelete() {
		...
		if (file_exists(DIR . '/events/onBeforeAuditStepDirectoryDelete.php')) {

			require_once(DIR . '/events/onBeforeAuditStepDirectoryDelete.php');
			onBeforeAuditStepDirectoryDelete($this);
	...
	public function doAfterDelete() {
		...
		if (file_exists(DIR . '/events/onAfterAuditStepDirectoryDelete.php')) {

			require_once(DIR . '/events/onAfterAuditStepDirectoryDelete.php');
			onAfterAuditStepDirectoryDelete($this);
	...
	private function writeFilterCache() {
		...
		$success = writeIntegerFilterCache('AuditStepDirectory',
				$this->id,
				'deleted',
				$this->deleted);
	    if (!$success) {
		...
		$success = writeStringFilterCache('AuditStepDirectory',
				$this->id,
				'step_action',
				$this->step_action);
	    if (!$success) {
	...
	private function removeFilterCache($removeOriginalFilters = true) {
        ...
        if ($removeOriginalFilters) {
		    $cacheFile = (DBDIR . '/AuditStepDirectory/' . $this->id . '.php');
	    ...
	    $success = removeIntegerFilterCache('AuditStepDirectory',
				$current->id,
				'deleted',
				$current->deleted);
	    if (!$success) {
		...
		$success = removeStringFilterCache('AuditStepDirectory',
				$current->id,
				'step_action',
				$current->step_action);
	    if (!$success) {
	...
	private function extractColumnValues() {
		...
		$expressionV0 = $this->getDisplayText('AuditStepDirectory/yes');

		$index++;
		$this->__columnValues[$index] = $expressionV0;

		$expressionV0 = $this->getDisplayText('AuditStepDirectory/no');
    ...
    /**
     * generateListFromFile - Generates list of AuditStepDirectory instances from file
    ...
    private function generateListFromFile() {
        ...
        if (($this->__filters['lastUpdateMinExclusive'] > 0)
            ...
            includeLibrary('extractIntegerBoundedSearchList');
            $currentResultIds = extractIntegerBoundedSearchList('AuditStepDirectory',
                    'LastUpdate',
        ...
        if ((isset($this->__filters['deletedInValues']))
            ...
            $currentResultIds = extractIntegerSearchList('AuditStepDirectory',
                    'Deleted',
        ...
        if ((isset($this->__filters['step_actionInValues']))
            ...
            $currentResultIds = extractStringSearchList('AuditStepDirectory',
                    'step_action',
        ...
        for ($i = 0; $i < $resultCount; $i++) {
            $object = new AuditStepDirectory(intval($resultIdKeys[$i]), true);
    ...
    private function getSQLQueryString() {
        ...
        $selectionSQL = 'SELECT * FROM `auditstepdirectorytable` ';
        $countSQL = 'SELECT COUNT(*) FROM `auditstepdirectorytable` ';
        $criteriaSQL = '';
        $sortOrderSQL = '';
