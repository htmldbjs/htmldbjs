	<div id="divUploadTaskPhotoDialog" class="divDialogContent" data-target-file-list="" data-media-type="">
		<div class="divContentWrapper level2">
			<div class="divDialogContentContainer">
				...
				<div class="divContentPanel z-depth-1 white">
					<form id="formUpload" name="formUpload" action="<?php echo $_SPRIT['URL_PREFIX']; ?>uploadtaskphoto/formupload" onsubmit="return false;" enctype="multipart/form-data">
						<input id="MAX_FILE_SIZE" name="MAX_FILE_SIZE" value="5120000" type="hidden">   
						<div class="divDropzonePanel">
							<div class="row">
								<button name="buttonUploadMedia" type="button" data-class-name="Task" class="buttonUploadMedia waves-effect waves-dark btn-large white blue-text text-darken-4 col s12">Dosya Yükle...</button>
							</div>
							<div id="divDropzone" class="divDropzone row dz-clickable" style="min-height: 400px;">
								<div class="col s12">
									...
									<div id="divUploaderInputContainer">
										<input accept=".3gp,.7z,.ae,.ai,.avi,.bmp,.cdr,.csv,.divx,.doc,.docx,.dwg,.eps,.flv,.gif,.gz,.ico,.iso,.jpg,.jpeg,.mov,.mp3,.mp4,.mpeg,.pdf,.png,.ppt,.ps,.psd,.rar,.svg,.swf,.tar,.tiff,.txt,.wav,.zip" data-class-name="Task" multiple="multiple" name="inputUploadMedia" class="inputUploadMedia inputUploader" style="display:none;" type="file">
									</div>
								</div>
							</div><!--.row-->
						</div>
					...
					<div class="row">
						<button id="buttonSelectMedia" name="buttonSelectMedia" type="button" data-text-pattern="%1 DOSYA SEÇİLDİ" data-zero-selection-pattern="DOSYA SEÇİLMEDİ" class="buttonSelectMedia waves-effect waves-light btn-large blue darken-4 col s12"></button>
					</div>
				</div>
			</div>
		</div>
	...
	<div class="divHiddenElements">
		<input type="text" id="uploaderActiveClass" value="Task">
