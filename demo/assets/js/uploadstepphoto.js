...
function initializeMediaHTMLDB() {
	...
	HTMLDB.initialize({
        ...
        readURL:(URLPrefix + "uploadstepphoto/readsession"),
        readAllURL:(URLPrefix + "uploadstepphoto/readsession"),
        writeURL:(URLPrefix + "uploadstepphoto/writesession"),
        writeDelay:1000,
        autoRefresh:0,
        renderElements:[],
...
function doMediaSessionHTMLDBRead() {
	...
	HTMLDB.initialize({
		...
		readURL:(URLPrefix + "uploadstepphoto/readmedia"),
		readAllURL:(URLPrefix + "uploadstepphoto/readmedia/all"),
		writeURL:"",
		writeDelay:1000,
		autoRefresh:0,
		renderElements:[],
...
function doAddStepPhotoButtonClick(sender) {
    if (!sender) {
        return;
    }
	
	rowId = sender.getAttribute("data-row-id");
	document.getElementById("uploadPhotoStepId").value = rowId;

	setHTMLDBFieldContents("divAuditStepHTMLDBReader", rowId);
    setHTMLDBFieldValues("divAuditStepHTMLDBReader", rowId);
	setHTMLDBFieldAttributes("divAuditStepHTMLDBReader", rowId);


	setFileListUL("ulphotosFileList");
	showDialog("divStepPhotoDialog");
}
...
function doSelectMediaButtonClick(element) {
    ...
    var id = 0;
    var fileGUID = "";
    var targetFileList = document.getElementById("divUploadStepPhotoDialog").getAttribute("data-target-file-list");
    ...
    hideDialog("divUploadStepPhotoDialog");
}
...
function doBrowseFileButtonClick(element) {
	...
	document.getElementById("divUploadStepPhotoDialog").setAttribute(
	...
	showDialog("divUploadStepPhotoDialog");
}
function filterMediaFiles() {
	var uploadedFilesUL = document.getElementById("ulUploadedFiles");
	var mediaType = parseInt(
			document.getElementById("divUploadStepPhotoDialog").getAttribute("data-media-type"));
...
function initializeDropzone() {
	...
	for (var i = 0; i < dropzoneElementCount; i++) {
		...
		dropzoneObject = new Dropzone(dropzoneElement, {
			"url": (URLPrefix + "uploadstepphoto/formupload"),
