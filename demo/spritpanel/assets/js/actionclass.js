...
function initializeHTMLDB() {
    ...
    HTMLDB.initialize({
        elementID:"divActionHTMLDB",
        readURL:(URLPrefix + "actionclass/read/all"),
        readAllURL:(URLPrefix + "actionclass/read/all"),
        validateURL:(URLPrefix + "actionclass/validate"),
        writeURL:(URLPrefix + "actionclass/write"),
        autoRefresh:0,
        renderElements:[],
        onReadAll:doHTMLDBActionReadAll,
        onRead:null,
        onWrite:doHTMLDBActionWrite,
        onRender:null,
        onRenderAll:null
    });

    HTMLDB.initialize({
        elementID:"divActionTableHTMLDB",
        readURL:(URLPrefix + "actionclass/readtable/all"),
        readAllURL:(URLPrefix + "actionclass/readtable/all"),
        writeURL:null,
        autoRefresh:0,
        renderElements:[{
        ...
        onWrite:null,
        onRender:null,
        onRenderAll:doHTMLDBActionRenderAll
    });

    HTMLDB.initialize({
        elementID:"divSessionHTMLDB",
        readURL:(URLPrefix + "actionclass/readsession"),
        readAllURL:(URLPrefix + "actionclass/readsession"),
        writeURL:(URLPrefix + "actionclass/writesession"),
        writeDelay:1000,
        autoRefresh:0,
        renderElements:[],
    ...
    HTMLDB.initialize({
        elementID:"divstep_idPropertyOptionsHTMLDB",
        readURL:(URLPrefix + "actionclass/readpropertyoptions/step_id"),
        readAllURL:(URLPrefix + "actionclass/readpropertyoptions/step_id"),
        writeURL:"",
        writeDelay:1000,
        autoRefresh:0,
        renderElements:[],
    ...
    HTMLDB.initialize({
        elementID:"divactionstate_idPropertyOptionsHTMLDB",
        readURL:(URLPrefix + "actionclass/readpropertyoptions/actionstate_id"),
        readAllURL:(URLPrefix + "actionclass/readpropertyoptions/actionstate_id"),
        writeURL:"",
        writeDelay:1000,
        autoRefresh:0,
        renderElements:[],
...
function doShowMoreButtonClick(sender) {
    ...
    HTMLDB.write("divSessionHTMLDB", false, function () {
        HTMLDB.read("divActionHTMLDB", false, function () {
            sender.disabled = false;
            HTMLDB.renderAll("divActionHTMLDB");
        });
    });
}
...
function searchList() {
    ...
    HTMLDB.write("divSessionHTMLDB", false, function () {
        ...
        resetProgressBar(
                ...
                false,
                true);
        document.getElementById("divActionHTMLDB_tbody").innerHTML = "";
        document.getElementById("divActionTableHTMLDB_tbody").innerHTML = "";
        document.getElementById("tbodyObjectList").innerHTML = "";
        document.getElementById("tbodyGhostObjectList").innerHTML = "";
        HTMLDB.read("divActionHTMLDB", true);
...
function doHTMLDBActionReadAll() {
    HTMLDB.read("divActionTableHTMLDB", true);
}
function doHTMLDBActionWrite() {
    HTMLDB.read("divActionHTMLDB", true);
}
function doHTMLDBActionRenderAll() {
    initializeObjects();
...
function validateObject(id) {
    var strHTMLDBDIVID = "divActionHTMLDB";
...
function saveObject(objObject) {
    ...
    var elTR = document.getElementById("divActionHTMLDB_tr" + id);
...
function doDeleteConfirmButtonClick(sender) {
    ...
    var strTR1Prefix = "divActionHTMLDB_tr";
    var strTR2Prefix = "divActionTableHTMLDB_tr";
...
function loadObject(id) {
	...
	document.getElementById("step_id").selectize.setValue(0);
	document.getElementById("actionstate_id").selectize.setValue(0);

    // Initialize Variable Values
    var arrClassProperties = HTMLDB.getColumnNames("divActionHTMLDB");
    var lClassPropertyCount = arrClassProperties.length;
    var elTR = document.getElementById("divActionHTMLDB_tr" + id);
    var objObject = null;
    ...
    if (elTR) {
        ...
        objObject = HTMLDB.get("divActionHTMLDB", id);
		if (parseInt(objObject["step_id"]) > 0) {
			document.getElementById("step_id").selectize.addOption(
					{value: objObject["step_id"],
					text: objObject["step_idDisplayText"]});
		}
		if (parseInt(objObject["actionstate_id"]) > 0) {
			document.getElementById("actionstate_id").selectize.addOption(
					{value: objObject["actionstate_id"],
					text: objObject["actionstate_idDisplayText"]});
		}
        for (var i = 0; i < lClassPropertyCount; i++) {
