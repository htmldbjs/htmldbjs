...
function initializeHTMLDB() {
    ...
    HTMLDB.initialize({
        elementID:"divFirmHTMLDB",
        readURL:(URLPrefix + "firmclass/read/all"),
        readAllURL:(URLPrefix + "firmclass/read/all"),
        validateURL:(URLPrefix + "firmclass/validate"),
        writeURL:(URLPrefix + "firmclass/write"),
        autoRefresh:0,
        renderElements:[],
        onReadAll:doHTMLDBFirmReadAll,
        onRead:null,
        onWrite:doHTMLDBFirmWrite,
        onRender:null,
        onRenderAll:null
    });

    HTMLDB.initialize({
        elementID:"divFirmTableHTMLDB",
        readURL:(URLPrefix + "firmclass/readtable/all"),
        readAllURL:(URLPrefix + "firmclass/readtable/all"),
        writeURL:null,
        autoRefresh:0,
        renderElements:[{
        ...
        onWrite:null,
        onRender:null,
        onRenderAll:doHTMLDBFirmRenderAll
    });

    HTMLDB.initialize({
        elementID:"divSessionHTMLDB",
        readURL:(URLPrefix + "firmclass/readsession"),
        readAllURL:(URLPrefix + "firmclass/readsession"),
        writeURL:(URLPrefix + "firmclass/writesession"),
        writeDelay:1000,
        autoRefresh:0,
        renderElements:[],
    ...
    HTMLDB.initialize({
        elementID:"divfirmstate_idPropertyOptionsHTMLDB",
        readURL:(URLPrefix + "firmclass/readpropertyoptions/firmstate_id"),
        readAllURL:(URLPrefix + "firmclass/readpropertyoptions/firmstate_id"),
        writeURL:"",
        writeDelay:1000,
        autoRefresh:0,
        renderElements:[],
...
function doShowMoreButtonClick(sender) {
    ...
    HTMLDB.write("divSessionHTMLDB", false, function () {
        HTMLDB.read("divFirmHTMLDB", false, function () {
            sender.disabled = false;
            HTMLDB.renderAll("divFirmHTMLDB");
        });
    });
}
...
function searchList() {
    ...
    HTMLDB.write("divSessionHTMLDB", false, function () {
        ...
        resetProgressBar(
                ...
                false,
                true);
        document.getElementById("divFirmHTMLDB_tbody").innerHTML = "";
        document.getElementById("divFirmTableHTMLDB_tbody").innerHTML = "";
        document.getElementById("tbodyObjectList").innerHTML = "";
        document.getElementById("tbodyGhostObjectList").innerHTML = "";
        HTMLDB.read("divFirmHTMLDB", true);
...
function doHTMLDBFirmReadAll() {
    HTMLDB.read("divFirmTableHTMLDB", true);
}
function doHTMLDBFirmWrite() {
    HTMLDB.read("divFirmHTMLDB", true);
}
function doHTMLDBFirmRenderAll() {
    initializeObjects();
...
function validateObject(id) {
    var strHTMLDBDIVID = "divFirmHTMLDB";
...
function saveObject(objObject) {
    ...
    var elTR = document.getElementById("divFirmHTMLDB_tr" + id);
...
function doDeleteConfirmButtonClick(sender) {
    ...
    var strTR1Prefix = "divFirmHTMLDB_tr";
    var strTR2Prefix = "divFirmTableHTMLDB_tr";
...
function loadObject(id) {
	...
	document.getElementById("firmstate_id").selectize.setValue(0);

    // Initialize Variable Values
    var arrClassProperties = HTMLDB.getColumnNames("divFirmHTMLDB");
    var lClassPropertyCount = arrClassProperties.length;
    var elTR = document.getElementById("divFirmHTMLDB_tr" + id);
    var objObject = null;
    ...
    if (elTR) {
        ...
        objObject = HTMLDB.get("divFirmHTMLDB", id);
		if (parseInt(objObject["firmstate_id"]) > 0) {
			document.getElementById("firmstate_id").selectize.addOption(
					{value: objObject["firmstate_id"],
					text: objObject["firmstate_idDisplayText"]});
		}
        for (var i = 0; i < lClassPropertyCount; i++) {
